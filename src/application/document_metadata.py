...
def _subject_from_text(text: str) -> tuple[str | None, str | None, str | None]:
...
def extract_document_metadata(
    ...
    *,
    ...
    # Filename is a title-only fallback; never use it to invent subject/grade.
    if not title:
    ...
    # Term is never taken from the filename or from overrides — only from what the document
    # itself says (via _term_from_text, or the LLM fallback echoing evidence it actually saw).
