...
class LibraryEntry(BaseModel):
    """One previously ingested book, as stored in the ingest dedup registry.

    Returned by GET /api/rag/books so a client can let a user pick a book they (or
    someone else) already uploaded before, and start querying it immediately with its
    file_reference_id — without re-uploading the file.
    """
    file_reference_id: str = Field(description="Unique file identity generated by the backend at upload time.")
    file_name: str = Field(description="Original filename as uploaded.")
    curriculum_id: str = Field(description="Resolved curriculum ID.")
    version: str = Field(description="Curriculum version.")
    chunks_created: int = Field(default=0, description="Number of child/parent chunks created.")
    indexed_chunks: int = Field(default=0, description="Number of vector indexed chunks.")
    indexed_question_count: int = Field(default=0, description="Number of indexed hypothetical questions.")
    language_counts: dict[str, int] = Field(default_factory=dict, description="Language distribution.")
    metadata_coverage: dict[str, float] = Field(default_factory=dict, description="Metadata extraction completeness percentages.")
    document_metadata: DocumentMetadataResponse | None = Field(default=None, description="Document metadata (title, subject, grade, etc.).")


class LibraryResponse(BaseModel):
    """List of every book previously ingested and still on record."""
    books: list[LibraryEntry] = Field(default_factory=list, description="Previously ingested books, most recent first.")


class ConversationTurn(BaseModel):
    """One persisted Q&A turn within a conversation, including the exact evidence
    and evaluation scores behind that specific answer."""
    turn: int = Field(description="1-based turn number within the conversation.")
    timestamp: str = Field(description="ISO-8601 UTC timestamp of this turn.")
    query: str = Field(description="The question as asked.")
    answer: str = Field(description="The generated answer.")
    answer_status: str = Field(description="Backend-determined answer status.")
    grounded: bool = Field(description="Whether the answer was judged grounded in evidence.")
    sources: list[ResponseSource] = Field(default_factory=list, description="Retrieved evidence chunks used for this turn.")
    retrieval: RetrievalSummary | None = Field(default=None, description="Retrieval diagnostics for this turn.")
    evaluation: EvaluationMetrics | None = Field(default=None, description="RAGAS evaluation scores for this turn.")


class ConversationDetail(BaseModel):
    """Full persisted conversation: the book it's bound to and every turn asked against it."""
    session_id: str = Field(description="Session ID this conversation is stored under.")
    file_reference_id: str = Field(description="The book this conversation is bound to.")
    file_name: str = Field(description="Human-readable file name of the book.")
    curriculum_id: str = Field(description="Resolved curriculum ID.")
    version: str = Field(description="Curriculum version.")
    created_at: str = Field(description="ISO-8601 UTC timestamp the conversation started.")
    updated_at: str = Field(description="ISO-8601 UTC timestamp of the last turn.")
    turns: list[ConversationTurn] = Field(default_factory=list, description="Every turn in chronological order.")


class ConversationSummary(BaseModel):
    """Lightweight listing entry for a conversation — no chunk text, for a fast browsable list."""
    session_id: str = Field(description="Session ID this conversation is stored under.")
    file_reference_id: str | None = Field(default=None, description="The book this conversation is bound to.")
    file_name: str | None = Field(default=None, description="Human-readable file name of the book.")
    curriculum_id: str | None = Field(default=None, description="Resolved curriculum ID.")
    version: str | None = Field(default=None, description="Curriculum version.")
    created_at: str | None = Field(default=None, description="ISO-8601 UTC timestamp the conversation started.")
    updated_at: str | None = Field(default=None, description="ISO-8601 UTC timestamp of the last turn.")
    turn_count: int = Field(default=0, description="Number of turns recorded.")
    last_query: str | None = Field(default=None, description="The most recent question asked.")


class ConversationListResponse(BaseModel):
    conversations: list[ConversationSummary] = Field(default_factory=list, description="Conversations, most recently updated first.")
