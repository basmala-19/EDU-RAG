...
CONTROL_CHARS = "\u0000\u0001\u0002\u0003\u0004\u0005\u0006\u0007\u0008\u000b\u000c\u000e\u000f\u0010\u0011\u0012\u0013\u0014\u0015\u0016\u0017\u0018\u0019\u001a\u001b\u001c\u001d\u001e\u001f"
_BIDI_MARKS = "\u200f\u200e\u202a\u202b\u202c\u2066\u2067\u2069\ufeff"
_CONTROL_CHAR_RE = re.compile(f"[{re.escape(CONTROL_CHARS + _BIDI_MARKS)}]")


def strip_control_chars(text: str) -> str:
    """Remove non-printable control/bidi-control characters from a single line of raw
    extracted text.

    Structural metadata (heading/chapter/lesson/section/topic) is built line-by-line as
    the PDF is walked, straight from each span's raw text via update_structure() —
    *before* normalize_text() ever runs (that only cleans the final assembled page
    body). Some embedded PDF fonts map bullet/icon glyphs to C0-control-range
    codepoints in their ToUnicode CMap (e.g. a "\\x07" bullet), which then survives
    untouched into `heading`/`chapter`/etc. Call this on each line right after
    extracting it, before it reaches detect_heading()/update_structure().
    """
    return _CONTROL_CHAR_RE.sub("", text or "")
...
def parse_numbered_heading(line: str, *, max_length: int = 180) -> tuple[int, str] | None:
    ...
    if ":" in title or "：" in title:
        return None
    # Cover pages routinely print an academic-year range on its own line, e.g.
    # "2026 - 2025". The regex above happily reads that as a level-1 numbered heading
    # ("2026" as the number, "2025" as the "title") once _clean_value() strips the
    # leading "-" from the remainder. Because a bare 4-digit year is never a real
    # heading *title* in any curriculum, reject it here — this previously let a cover
    # page's year range become the permanent `chapter` value for the entire document,
    # since nothing later in the book ever produced another level-1 positional heading
    # to overwrite it.
...
_AR_UNIT_ORDINAL_TO_NUM: dict[str, int] = {
...
_EN_UNIT_ORDINAL_TO_NUM: dict[str, int] = {
    "first": 1, "second": 2, "third": 3, "fourth": 4, "fifth": 5,
    "sixth": 6, "seventh": 7, "eighth": 8, "ninth": 9, "tenth": 10,
    "eleventh": 11, "twelfth": 12,
    # "Unit One" / "Unit Two" (cardinal) is at least as common as "Unit First" in
    # English curriculum covers, so both word forms are accepted.
    "one": 1, "two": 2, "three": 3, "four": 4, "five": 5,
    "six": 6, "seven": 7, "eight": 8, "nine": 9, "ten": 10,
    "eleven": 11, "twelve": 12,
}
# Longest-first so "الثانية عشرة" is tried before the shorter "الثانية" it starts with.
_AR_UNIT_ORDINAL_GROUP = "|".join(
    re.escape(w) for w in sorted(_AR_UNIT_ORDINAL_TO_NUM, key=len, reverse=True)
)
_EN_UNIT_ORDINAL_GROUP = "|".join(re.escape(w) for w in _EN_UNIT_ORDINAL_TO_NUM)

_UNIT_HEADING_RE = re.compile(
    rf"^(?:"
    rf"الوحدة(?:\s+الدراسية)?\s+(?P<ar_word>{_AR_UNIT_ORDINAL_GROUP})|"
    rf"الوحدة(?:\s+الدراسية)?\s+(?:رقم\s+)?(?P<ar_digit>\d{{1,2}})|"
    rf"unit\s+(?P<en_word>{_EN_UNIT_ORDINAL_GROUP})|"
    rf"unit\s+(?:no\.?\s*)?(?P<en_digit>\d{{1,2}})"
    rf")\s*(?:[:\-–—]\s*(?P<title>.+))?$",
    flags=re.I,
)


def parse_unit_heading(line: str) -> tuple[str, str] | None:
    """Detect "الوحدة الأولى" / "الوحدة الدراسية الثانية" / "Unit One" / "Unit 3" —
    this curriculum's actual chapter-level marker. LABEL_PATTERNS already lists "الوحدة"
    as a chapter label, but _label_match() only fires on "<label> <separator> <value>"
    (e.g. "الوحدة: ..."); a bare "الوحدة الأولى" with no colon/dash between the word and
    the ordinal never matched that shape, so books that head every unit this way (verified
    on pages 7, 16, 29, ...) left `chapter` permanently None even though the structure was
    printed right there.

    Returns (title, normalized_chapter): `title` is the literal line text (or the part
    after a separator, if present) — kept as-is for `heading`/`heading_path` so nothing
    printed is lost. `normalized_chapter` is a language-neutral "Chapter N" label, so
    `chapter` groups/sorts consistently across books that call this level "الوحدة",
    "Unit", or "الفصل"/"Chapter", instead of three different literal strings for the same
    position in the hierarchy.
    """
    s = _clean_value(line)
    if not s:
        return None
    m = _UNIT_HEADING_RE.match(s)
    if not m:
        return None
    if m.group("ar_word"):
        num = _AR_UNIT_ORDINAL_TO_NUM[m.group("ar_word")]
    elif m.group("ar_digit"):
        num = int(m.group("ar_digit"))
    elif m.group("en_word"):
        num = _EN_UNIT_ORDINAL_TO_NUM[m.group("en_word").casefold()]
    else:
        num = int(m.group("en_digit"))
    title = _clean_value(m.group("title")) or s
    return title, f"Chapter {num}"
...
def update_structure(
    ...
    *,
    ...
    `toc_index`, when given, is the book's own parsed table of contents (see
    `src.application.toc.TocIndex`). It is assistive only — an explicit label ("Chapter:
    ...") or an unambiguous structural marker ("الوحدة الأولى") always wins over it, exactly
    as before this parameter existed. It is only consulted where the body text alone leaves
    real ambiguity: correcting a numbered heading's chapter/lesson level, and recognising a
    heading line that carries no numbering or label at all.
    """
    changed = False
    ...
    unit_match = parse_unit_heading(line)
    if unit_match:
        title, chapter_norm = unit_match
        prev_heading = state.get("heading")
        prev_chapter = state.get("chapter")
        state["heading"] = title
        state["heading_level"] = 1
        state["heading_path"] = [title]
        if "chapter" not in state.get("_explicit_hierarchy_fields", set()):
            state["chapter"] = chapter_norm
        return prev_heading != title or state.get("chapter") != prev_chapter
    ...
    parsed = detect_heading_ex(line, markdown_level=markdown_level, max_length=max_length)
    if not parsed:
        return _apply_toc_only_heading(state, line, toc_index)
    level, title, positional_eligible = parsed
    # A numbered heading's dot-count is a reliable *depth* signal but only a guess at
    # which semantic level (chapter vs. lesson) that depth corresponds to in this
    # particular book. When the book's own TOC classifies this exact title as a unit/
    # chapter or a lesson, trust that over the raw numbering depth — restricted to the
    # chapter/lesson distinction (levels 1/2) since that's as far as a TOC reliably goes;
    # deeper section/topic levels keep the numbering-derived level as before. The heading
    # text itself is never replaced by the TOC's wording — only the level classification.
    if toc_index is not None and level in (1, 2):
        toc_level = toc_index.classify_level(title)
        if toc_level in (1, 2):
            level = toc_level
    prev = state.get("heading")
    path = split_heading_path(state.get("heading_path"))
...
def _apply_toc_only_heading(state: dict[str, Any], line: str, toc_index: "TocIndex | None") -> bool:
    """Last-resort heading detection: a line with no numbering, markdown marker, or label
    of its own, but whose text strongly matches an entry in the book's own TOC — the case
    a standalone bold/large-font chapter or lesson title with no printed number falls
    into, which every other detector above necessarily misses since there's no structural
    marker to key off in the text alone. Only fires when `toc_index` was supplied and the
    match is unambiguous (see TocIndex.match's default threshold); the heading's own
    printed wording is kept as-is, never replaced by the TOC's phrasing."""
    if toc_index is None:
        return False
    stripped = _clean_value(line)
    if not stripped or len(stripped) > 140:
        return False
    # Higher bar than the level-correction use above: with no numbering/label of its own,
    # this line's only evidence of being a heading at all *is* the TOC match, so it must
    # be a strong one to avoid mistaking an ordinary sentence that happens to resemble a
    # TOC entry for an actual heading.
    found = toc_index.match(stripped, min_ratio=0.82)
    if not found:
        return False
    entry, _score = found
    if entry.level not in (1, 2):
        return False
    prev_heading = state.get("heading")
    prev_path = split_heading_path(state.get("heading_path"))
    level = entry.level
    path = prev_path[: level - 1] + [stripped]
    state["heading"] = stripped
    state["heading_level"] = level
    state["heading_path"] = path
    positional_field = {1: "chapter", 2: "lesson"}[level]
    if positional_field not in state.get("_explicit_hierarchy_fields", set()):
        state[positional_field] = stripped
    return prev_heading != stripped or path != prev_path
...
def extract_hierarchy(
    text: str, seed: dict[str, Any] | None = None, *, toc_index: "TocIndex | None" = None
) -> dict[str, Any]:
