...
from src.features.rag.domain.schemas import (
    ConversationDetail,
...
from src.features.rag.infrastructure.document_loader import SUPPORTED_EXTENSIONS
from src.features.rag.application.ingestion_service import IngestionService
from src.features.rag.application.ingestion_jobs import IngestionJobService
from src.features.rag.application.generation import build_context, generate_with_provider
from src.features.rag.application.retrieval_service import RetrievalService
from src.features.rag.application.evaluation import RAGEvaluator
from src.features.rag.infrastructure.config import get_settings
from src.features.rag.infrastructure.health import health
from src.features.rag.infrastructure.ingest_registry import IngestRegistry
from src.features.rag.infrastructure.conversation_store import ConversationStore
from src.features.rag.application.metadata import clean_optional, split_heading_path
from src.features.rag.application.session import LearningSessionStore
...
app = FastAPI(
    ...
    openapi_tags=[
        ...
        {"name": "retrieval", "description": "Retrieve ranked evidence chunks for indexed curriculum files."},
...
# Configurable because some Windows setups inherit a read-only ACL on
# ``data/raw`` even though the application itself is otherwise writable.
# The unified launcher supplies a writable directory for this value.
...
async def _warm_up_models() -> None:
    ...
    def _load() -> None:
        ...
        try:
            ...
            if settings.reranker_enabled and settings.reranker_backend.casefold() == "local":
                from src.features.rag.infrastructure.ranking import _get_reranker
    ...
    # Do not delay the API becoming available while a remote embedding provider
    # is slow or temporarily unreachable. Health and retrieval endpoints report
    # the actual backend state, while this background task only warms caches.
    asyncio.create_task(run_in_threadpool(_load))
    settings = get_settings()
...
@app.post(
    ...
    tags=["retrieval"],
    summary="Retrieve grounded curriculum chunks",
    description=(
        "Returns ranked source chunks for one indexed file without generating an answer. "
        "The file_reference_id is mandatory and is always enforced as a retrieval filter."
    ),
)
def rag_retrieve(request: RetrievalRequest) -> RetrievalResponse:
    """Expose the shared hybrid retriever for consumers such as question generation."""
    try:
        return retrieval_service.retrieve_request(request)
    except ValueError as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc
    except Exception as exc:
        logger.exception("Retrieval processing failed")
        raise HTTPException(status_code=500, detail="Retrieval processing failed. Check server logs for details.") from exc
...
def rag_response(request: ResponseRequest) -> ResponsePayload:
    ...
    try:
        ...
        def _clean_src_text(val: Any) -> str | None:
            if not val:
                return None
            from src.features.rag.infrastructure.ar_text import repair_ocr_artifacts
