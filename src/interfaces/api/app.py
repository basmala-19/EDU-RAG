...
from src.application.curriculum_structure import CurriculumStructureService
...
app = FastAPI(
    ...
    description=(
        ...
        "`file_reference_id` + a question to get a grounded, evidence-cited answer.\n\n"
        "A live testing console (chat + clickable evidence sources) is available at "
        "[`/console`](/console)."
    ),
    ...
    openapi_tags=[
        ...
        {"name": "discovery", "description": "Look up what subjects/grades/chapters/lessons are actually indexed."},
...
@app.get(
    "/api/rag/structure",
    response_model=CurriculumStructureResponse,
    tags=["discovery"],
    summary="Discover indexed subjects, grades, chapters and lessons",
    description=(
        "A caller building a filtered /api/rag/response call (chapter/lesson) can't be "
        "expected to already know the exact spelling stored for a given book — this "
        "endpoint is the lookup for that, progressively:\n\n"
        "- No params: every indexed subject, with the grades indexed under each.\n"
        "- subject only: every grade indexed under that subject.\n"
        "- subject + grade: the chapter/lesson tree for that subject+grade, in book order.\n\n"
        "Values must match indexed metadata exactly (case-sensitive) — that's the point: "
        "walk the discovery chain instead of guessing. Works the same for any indexed "
        "book, Arabic or English; nothing here is specific to one curriculum."
    ),
    responses={
        200: {
            "model": CurriculumStructureResponse,
            "description": "Requested level of the subject/grade/chapter/lesson tree.",
            "content": {
                "application/json": {
                    "examples": {
                        "subjects": {
                            "summary": "No params",
                            "value": {"subjects": [{"subject": "تكنولوجيا المعلومات والاتصالات", "grades": ["الأول"]}]},
                        },
                        "grades": {
                            "summary": "subject only",
                            "value": {"subject": "تكنولوجيا المعلومات والاتصالات", "grades": ["الأول"]},
                        },
                        "chapters": {
                            ...
                            "value": {
                                ...
                                "grade": "الأول",
                                "chapters": [{"chapter": "الذكاء الاصطناعي", "lessons": ["التعلم الآلي", "التعلم العميق"]}],
                            },
                        },
                    }
                }
...
def rag_structure(subject: str | None = None, grade: str | None = None) -> CurriculumStructureResponse:
    subject = clean_optional(subject)
    grade = clean_optional(grade)
    try:
        if not subject:
            return CurriculumStructureResponse(
                subjects=[SubjectStructure(**s) for s in structure_service.list_subjects()]
            )
        if not grade:
            return CurriculumStructureResponse(subject=subject, grades=structure_service.list_grades(subject))
        return CurriculumStructureResponse(
            subject=subject,
            grade=grade,
            chapters=[ChapterStructure(**c) for c in structure_service.list_structure(subject, grade)],
        )
    except Exception as exc:
        logger.exception("Structure lookup failed")
        raise HTTPException(status_code=500, detail="Structure lookup failed. Check server logs for details.") from exc
...
def rag_response(request: ResponseRequest) -> ResponsePayload:
    ...
    try:
        ...
        def _run_retrieval(query_text: str) -> list[dict]:
            ...
            if not results and scoped_filters != base_filters:
                # Metadata for chapter/lesson can be incomplete for some documents; don't let
                # an imperfect ingestion tag hide evidence that's genuinely in the file.
                retrieved = retrieval_service.retrieve(query_text, base_filters, request.top_k)
                results = [r.model_dump() for r in retrieved.results]
            return results
        ...
        reranker_score = float(top_meta.get("reranker_score", results[0].get("score", 0.0)))
        retrieval_confidence = float(top_meta.get("retrieval_confidence", results[0].get("score", 0.0)))
        ...
        # Stricter, informational bar used only below as a fallback when the model didn't
        ...
        retrieval_gate_passed = reranker_score >= 0.42 and retrieval_confidence >= settings.min_grounding_score
