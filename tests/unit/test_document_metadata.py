...
def test_arabic_opening_document_pattern():
    ...
    # This cover text never mentions a term/semester explicitly, so term must stay
    # null rather than being guessed from the "T1" in the filename.
...
def test_term_never_guessed_from_filename():
    """A filename like '..._T2.pdf' must never leak into term when the document body
    itself says nothing about a term/semester."""
